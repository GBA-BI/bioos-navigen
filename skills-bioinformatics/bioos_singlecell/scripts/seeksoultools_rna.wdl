...
workflow SeekSoulToolsFastWorkflow {
  ...
  output {
    File result_tar = SeekSoulToolsRNARun.result_tar
    File stdout_log = SeekSoulToolsRNARun.stdout_log
    File stderr_log = SeekSoulToolsRNARun.stderr_log
    File summary_json = SeekSoulToolsRNARun.summary_json
    File summary_csv = SeekSoulToolsRNARun.summary_csv
    File report_html = SeekSoulToolsRNARun.report_html
    File step1_file = SeekSoulToolsRNARun.step1_file
    File step2_file = SeekSoulToolsRNARun.step2_file
    File step3_file = SeekSoulToolsRNARun.step3_file
    File step4_file = SeekSoulToolsRNARun.step4_file
    File raw_feature_bc_matrix = SeekSoulToolsRNARun.raw_feature_bc_matrix
    File filtered_feature_bc_matrix = SeekSoulToolsRNARun.filtered_feature_bc_matrix
  }
}

task SeekSoulToolsRNARun {
  input {
    File fq1
    File fq2
    String samplename
